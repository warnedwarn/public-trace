...
def main():
    ...
    account=create_account(account_private_key=key);client=create_client(chain=testnet_bradbury,account=account);code=open(os.path.join(ROOT,'contracts','contract.py'),encoding='utf-8').read()
    ...
    out={'contract':address,'deployTx':h,'network':'testnet-bradbury','deployer':account.address};open(os.path.join(ROOT,'deployment.json'),'w',encoding='utf-8').write(json.dumps(out,indent=2));print(json.dumps(out),flush=True)
