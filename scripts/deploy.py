...
def main():
    ...
    h=client.deploy_contract(code=code,args=[]);print('deployTx',h,flush=True);receipt=client.wait_for_transaction_receipt(transaction_hash=h,status='FINALIZED',retries=180,interval=5000);address=find(receipt)
    if not address:raise SystemExit('No contract address in finalized receipt')
